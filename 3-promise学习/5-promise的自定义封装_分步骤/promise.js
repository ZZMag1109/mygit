// 封装成一个类
class Promise {
    // 构造方法,new一个类（Promise）的实例即实例化一个Promise对象的时候，会立即执行构造函数
    ...
    // then方法是普通方法，将会放在Promise的原型对象上，供实例调用
    ...
    // catch方法是普通方法，将会放在Promise的原型对象上，供实例调用
    catch(onRejected) {
        return this.then(undefined, onRejected);
    }

    // resolve方法是类（promise对象）的静态方法，不是示例的方法
    ...
    // reject方法是类（promise对象）的静态方法，不是示例的方法
    ...
    // all方法是类（promise对象）的静态方法，不是示例的方法
    static all(promises) {
        // 申明变量
        let count = 0;
        // 申明一个数组，存放成功的结果的数组
    ...
    // race方法是类（promise对象）的静态方法，不是示例的方法
